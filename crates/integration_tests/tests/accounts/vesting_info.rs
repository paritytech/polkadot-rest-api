...
async fn test_vesting_info_basic() -> Result<()> {
    ...
    println!("  {} Response contains {} vesting schedule(s)", "✓".green(), vesting.len());

    for (i, schedule) in vesting.iter().enumerate() {
        let schedule_obj = schedule.as_object().unwrap();
        assert!(schedule_obj.contains_key("locked"), "Schedule {} missing 'locked' field", i);
        assert!(schedule_obj.contains_key("perBlock"), "Schedule {} missing 'perBlock' field", i);
        assert!(schedule_obj.contains_key("startingBlock"), "Schedule {} missing 'startingBlock' field", i);
...
async fn test_vesting_info_with_include_claimable() -> Result<()> {
    ...
    if !vesting.is_empty() {
        assert!(response_obj.contains_key("vestedBalance"), "Response missing 'vestedBalance' field");
        assert!(response_obj.contains_key("vestingTotal"), "Response missing 'vestingTotal' field");
        assert!(response_obj.contains_key("vestedClaimable"), "Response missing 'vestedClaimable' field");
        assert!(response_obj.contains_key("blockNumberForCalculation"), "Response missing 'blockNumberForCalculation' field");
        assert!(response_obj.contains_key("blockNumberSource"), "Response missing 'blockNumberSource' field");
...
async fn test_vesting_info_schedule_structure() -> Result<()> {
    ...
    println!("  {} Account has {} vesting schedule(s)", "ℹ".blue(), vesting.len());
    ...
    for (i, schedule) in vesting.iter().enumerate() {
        ...
        assert!(schedule_obj.get("locked").unwrap().is_string(), "Schedule {} 'locked' should be a string", i);
        assert!(schedule_obj.get("perBlock").unwrap().is_string(), "Schedule {} 'perBlock' should be a string", i);
        assert!(schedule_obj.get("startingBlock").unwrap().is_string(), "Schedule {} 'startingBlock' should be a string", i);
        ...
        assert!(locked.parse::<u128>().is_ok(), "Schedule {} 'locked' is not a valid number", i);
        assert!(per_block.parse::<u128>().is_ok(), "Schedule {} 'perBlock' is not a valid number", i);
        assert!(starting_block.parse::<u64>().is_ok(), "Schedule {} 'startingBlock' is not a valid number", i);

        println!("    Schedule {}: locked={}, perBlock={}, startingBlock={}", i, locked, per_block, starting_block);
    }
...
async fn test_vesting_info_use_rc_block() -> Result<()> {
    ...
    for (i, item) in local_array.iter().enumerate() {
        ...
        assert!(item_obj.contains_key("vesting"), "Item {} missing 'vesting'", i);
...
async fn test_vesting_info_use_rc_block_with_include_claimable() -> Result<()> {
    ...
    for (i, item) in local_array.iter().enumerate() {
        ...
        if !vesting.is_empty() {
            assert!(item_obj.contains_key("vestedBalance"), "Item {} missing 'vestedBalance'", i);
            assert!(item_obj.contains_key("vestingTotal"), "Item {} missing 'vestingTotal'", i);
            assert!(item_obj.contains_key("vestedClaimable"), "Item {} missing 'vestedClaimable'", i);

            if let Some(source) = item_obj.get("blockNumberSource") {
                let source_str = source.as_str().unwrap_or("");
                assert_eq!(source_str, "relay", "When useRcBlock=true, blockNumberSource should be 'relay'");
            }
        }
    }

    println!("{} All {} block response(s) with includeClaimable validated!", "✓".green().bold(), local_array.len());
    println!("{}", "═".repeat(80).bright_white());
    Ok(())
}
