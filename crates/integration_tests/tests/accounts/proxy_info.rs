...
async fn test_proxy_info_basic() -> Result<()> {
    ...
    assert!(response_obj.contains_key("delegatedAccounts"), "Response missing 'delegatedAccounts' field");
    assert!(response_obj.contains_key("depositHeld"), "Response missing 'depositHeld' field");
    ...
    let delegated_accounts = response_obj.get("delegatedAccounts").unwrap().as_array().unwrap();
    ...
    for (i, proxy) in delegated_accounts.iter().enumerate() {
        ...
        assert!(proxy_obj.contains_key("delegate"), "Proxy {} missing 'delegate' field", i);
        assert!(proxy_obj.contains_key("proxyType"), "Proxy {} missing 'proxyType' field", i);
        assert!(proxy_obj.contains_key("delay"), "Proxy {} missing 'delay' field", i);
...
async fn test_proxy_info_no_proxies() -> Result<()> {
    ...
    if delegated_accounts.is_empty() {
        assert_eq!(deposit_held, "0", "Expected deposit to be 0 when no proxies");
    }
...
async fn test_proxy_info_use_rc_block() -> Result<()> {
    ...
    for (i, item) in local_array.iter().enumerate() {
        ...
        assert!(item_obj.contains_key("delegatedAccounts"), "Item {} missing 'delegatedAccounts'", i);
        assert!(item_obj.contains_key("depositHeld"), "Item {} missing 'depositHeld'", i);
