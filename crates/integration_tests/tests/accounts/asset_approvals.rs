...
async fn test_asset_approvals_missing_required_params() -> Result<()> {
    ...
    assert_eq!(response_no_asset.status.as_u16(), 400, "Expected 400 Bad Request for missing assetId");
    assert!(response_no_asset.body.contains("assetId"), "Error message should mention missing assetId");
    ...
    assert_eq!(response_no_delegate.status.as_u16(), 400, "Expected 400 Bad Request for missing delegate");
    assert!(response_no_delegate.body.contains("delegate"), "Error message should mention missing delegate");
