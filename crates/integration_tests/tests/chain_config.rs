...
fn test_relay_chains_have_correct_config() {
    ...
    for chain_name in &["polkadot", "kusama", "westend"] {
        ...
        assert_eq!(config.chain_type, ChainType::Relay, "{} should be relay chain", chain_name);
        assert_eq!(config.legacy_types, "polkadot", "{} should use polkadot legacy types", chain_name);
        assert!(config.finalizes, "{} should finalize", chain_name);
        assert_eq!(config.relay_chain, None, "{} should not have relay_chain", chain_name);
        assert_eq!(config.para_id, None, "{} should not have para_id", chain_name);
    }
}
...
fn test_asset_hubs_have_correct_config() {
    ...
    for (chain_name, expected_relay, expected_para_id) in asset_hub_configs {
        ...
        assert_eq!(config.chain_type, ChainType::AssetHub, "{} should be asset hub", chain_name);
        assert_eq!(config.legacy_types, "none", "{} should use no legacy types", chain_name);
        assert_eq!(
            config.relay_chain,
...
fn test_fee_config_values_are_reasonable() {
    ...
    let polkadot = configs.get("polkadot").unwrap();
    assert_eq!(polkadot.min_calc_fee_runtime, 0, "Polkadot min_calc_fee_runtime should be 0");
