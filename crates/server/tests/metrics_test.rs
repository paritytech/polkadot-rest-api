...
fn test_metrics_initialization() {
    ...
    // Should contain the sas_ prefix
    assert!(metrics_text.contains("sas_"));

    // Should contain our custom metrics
    assert!(metrics_text.contains("sas_http_requests"));
    assert!(metrics_text.contains("sas_request_duration_seconds"));

    // Response size bytes histogram won't appear until data is recorded
    // Just check that we can gather metrics without error
}
