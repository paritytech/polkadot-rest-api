...
pub fn extract_para_inclusions_from_events(
    ...
    for event_result in events.iter() {
        ...
        // Decode event fields using typed DecodeAsType struct
        ...
        if let Some(inclusion) = extract_inclusion_from_typed(&event_fields) {
...
/// Extract inclusion data from typed CandidateIncluded event
fn extract_inclusion_from_typed(event: &CandidateIncludedEvent) -> Option<ParaInclusion> {
    let descriptor = &event.candidate.descriptor;

    // Extract head data to get para block number and hash
    let (para_block_number, para_block_hash) = extract_head_data(&event.head_data)?;

    Some(ParaInclusion {
        para_id: descriptor.para_id.to_string(),
        para_block_number: para_block_number.to_string(),
        para_block_hash,
        descriptor: CandidateDescriptor {
            relay_parent: format!("0x{}", hex::encode(descriptor.relay_parent)),
            persisted_validation_data_hash: format!(
                "0x{}",
                hex::encode(descriptor.persisted_validation_data_hash)
            ),
            pov_hash: format!("0x{}", hex::encode(descriptor.pov_hash)),
            erasure_root: format!("0x{}", hex::encode(descriptor.erasure_root)),
            para_head: format!("0x{}", hex::encode(descriptor.para_head)),
            validation_code_hash: format!("0x{}", hex::encode(descriptor.validation_code_hash)),
        },
        commitments_hash: format!("0x{}", hex::encode(event.candidate.commitments_hash)),
        core_index: event.core_index.to_string(),
        group_index: event.group_index.to_string(),
    })
}
