...
/// Error types for /blocks/{blockId} endpoint
#[derive(Debug, Error)]
...
async fn get_validators_at_block(
    ...
    let validators_value = plain_entry
        .fetch()
        .await?
...
/// Fetch block response given a block hash and number
///
/// This function contains the logic for fetching and formatting block data.
/// It can be called by any handler that needs block information.
pub async fn fetch_block_response(
    state: &AppState,
    block_hash: &str,
    block_number: u64,
...
pub async fn get_block(
    ...
    // Fetch block data
    let response =
        fetch_block_response(&state, &resolved_block.hash, resolved_block.number).await?;
