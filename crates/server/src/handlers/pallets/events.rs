//! Handler for `/pallets/{palletId}/events` endpoint.

use crate::handlers::pallets::common::{
    AtResponse, FieldMetadata, PalletError, PalletQueryParams, RcBlockFields, expand_type_to_string,
    find_pallet_v14, find_pallet_v15,
};
...
pub struct PalletsEventsResponse {
    pub at: AtResponse,
    pub pallet: String,
...
pub async fn get_pallets_events(
...
) -> Result<PalletsEventsResponse, PalletError> {
    match metadata {
        RuntimeMetadata::V14(meta) => extract_events_v14(meta, pallet_id, at, only_ids, rc_fields),
        RuntimeMetadata::V15(meta) => extract_events_v15(meta, pallet_id, at, only_ids, rc_fields),
...
fn extract_events_v14(
    ...
    let items = if let Some(event) = &pallet.event {
        extract_event_variants(&meta.types, event.ty.id, only_ids)
    } else if only_ids {
        ...
        EventsItems::Full(
            variant_def
                ...
                .map(|v| {
                    ...
                    // Expand types to JSON-like strings for args (like Sidecar)
                    let args: Vec<String> = v
                        .fields
                        .iter()
                        .map(|f| expand_type_to_string(types, f.ty.id))
                        .collect();
