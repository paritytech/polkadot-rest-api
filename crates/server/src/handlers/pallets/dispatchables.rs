...
/// Simplify type name by removing `<T>` suffix and other generic markers
fn simplify_type_name(type_name: &str) -> String {
    ...
    // Extract just the type name part (after last ::)
    simplified
        .rsplit("::")
        .next()
...
fn format_type_def_v14(registry: &scale_info::PortableRegistry, ty: &scale_info::Type<scale_info::form::PortableForm>) -> String {
    ...
    match &ty.type_def {
        ...
        TypeDef::Variant(_) => {
            // Use the type path for variants/enums
            ty.path.segments.last()
                .map(|s| s.to_string())
        ...
        TypeDef::Tuple(tuple) => {
            if tuple.fields.is_empty() {
                ...
                let fields: Vec<String> = tuple.fields
                    .iter()
...
fn extract_dispatchables_v9(
    ...
    let items = if only_ids {
        DispatchablesItems::OnlyIds(calls.iter().map(|c| extract_str(&c.name).to_string()).collect())
    } else {
...
fn extract_dispatchable_item_v9(
    ...
    let calls = match &module.calls {
        Some(DecodeDifferent::Decoded(calls)) => calls,
        _ => return Err(PalletError::DispatchableNotFound(dispatchable_id.to_string())),
    };
...
fn extract_dispatchables_v14(
    ...
    let calls_type = meta
        ...
        .ok_or_else(|| PalletError::PalletNotFound(format!("Could not resolve calls type for {}", pallet_id)))?;
...
fn extract_dispatchable_item_v14(
    ...
    let calls_type_id = match &pallet.calls {
        Some(calls) => calls.ty.id,
        None => return Err(PalletError::DispatchableNotFound(dispatchable_id.to_string())),
    };
...
fn extract_dispatchables_v16(
    ...
    let (pallet_name, pallet_index) =
        find_pallet_v16(&meta.pallets, pallet_id).ok_or_else(|| PalletError::PalletNotFound(pallet_id.to_string()))?;
...
mod tests {
    ...
    fn test_dispatchables_items_serialization() {
        let items = DispatchablesItems::OnlyIds(vec!["transfer".to_string(), "approve".to_string()]);
