...
pub async fn runtime_metadata_versions(
    ...
    // Fetch metadata versions at the specified block hash using state_call
    // The result comes back as a hex-encoded string that needs to be decoded
    let versions_hex: String = state
        .rpc_client
        .request("state_call", subxt_rpcs::rpc_params!["Metadata_metadata_versions", "0x", &block_hash_str])
        .await
        .map_err(GetMetadataVersionsError::MetadataVersionsFailed)?;

    // Decode the hex string to get the versions
    // The format is: 4 bytes (little-endian) for each version number
    ...
    // Parse as little-endian u32 values
    ...
    let response = MetadataVersionsResponse {
        at: BlockInfo {
            hash: block_hash_str,
            height: block_height,
        },
