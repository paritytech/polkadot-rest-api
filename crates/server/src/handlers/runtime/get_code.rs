...
// The storage key for the Wasm code in Substrate
// Ref: https://github.com/paritytech/substrate/blob/master/frame/system/src/lib.rs
...
pub async fn runtime_code(
    ...
    // Fetch the runtime code (Wasm blob) at the specified block hash
    // state_getStorage returns the SCALE-encoded data, we'll get it as a hex string
    let code: Option<String> = state
        .rpc_client
        .request("state_getStorage", subxt_rpcs::rpc_params![CODE_STORAGE_KEY, &block_hash_str])
        .await
        .map_err(GetCodeError::CodeFailed)?;

    // If code is None, return an empty string; otherwise return the hex-encoded code
    let code_value = code.unwrap_or_else(|| "0x".to_string());
