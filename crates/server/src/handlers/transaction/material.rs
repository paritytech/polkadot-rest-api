...
pub enum MetadataValue {
    Scale(String),
    Json(Box<frame_metadata::RuntimeMetadataPrefixed>),
}
    ...
    let metadata = if let Some(format) = metadata_format {
        ...
        match (format, metadata_bytes) {
            ...
            (MetadataFormat::Scale, Some(bytes)) => {
                Some(MetadataValue::Scale(format!("0x{}", hex::encode(&bytes))))
            }
            (MetadataFormat::Json, Some(bytes)) => {
                ...
                Some(MetadataValue::Json(Box::new(metadata)))
            }
        }
    } else {
        None
...
async fn material_internal(
    ...
    let metadata = if let Some(format) = metadata_format {
        ...
        match format {
            MetadataFormat::Scale => Some(MetadataValue::Scale(metadata_hex)),
            MetadataFormat::Json => {
                // Decode the metadata
                let metadata_bytes =
...
mod tests {
    ...
    fn test_material_response_with_metadata_serialization() {
        let response = MaterialResponse {
            ...
            metadata: Some(MetadataValue::Scale("0xmetadata".to_string())),
        };
        let json = serde_json::to_value(&response).unwrap();
