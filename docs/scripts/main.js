...
class DocApp {
    ...
    setupUI() {
        ...
        // Setup Send Request handler
        this.setupSendRequest();
            ...
            if (navGroupHeader) {
                ...
                if (target && navGroupHeader) {
                    ...
                    if (isExpanded) {
                        ...
                        // Collapse all other expanded groups (accordion)
                        document.querySelectorAll('[data-toggle^="nav-group-"].expanded').forEach(otherHeader => {
                            if (otherHeader !== navGroupHeader) {
                                const otherId = otherHeader.dataset.toggle;
                                const otherTarget = document.getElementById(otherId);
                                if (otherTarget) {
                                    otherTarget.style.maxHeight = '0px';
                                    otherTarget.style.overflow = 'hidden';
                                }
    ...
    /**
     * Setup Send Request button handler
     */
    setupSendRequest() {
        document.addEventListener('click', async (e) => {
            if (e.target.matches('.send-request-btn') || e.target.closest('.send-request-btn')) {
                e.preventDefault();
                const button = e.target.matches('.send-request-btn') ? e.target : e.target.closest('.send-request-btn');
                const endpointId = button.dataset.endpointId;
                ...
                // Build URL from current server and parameter inputs
                const config = this.buildApiExplorerRequestConfig(endpoint, endpointId);

                // Show loading state
                button.disabled = true;
                button.textContent = 'Sending...';

                const responseSection = document.getElementById(`response-output-${endpointId}`);
                const statusEl = document.getElementById(`response-status-${endpointId}`);
                const timeEl = document.getElementById(`response-time-${endpointId}`);
                ...
                try {
                    ...
                    if (statusEl) {
                        ...
                        statusEl.className = `response-status ${response.ok ? 'status-success' : 'status-error'}`;
                    }
                    if (timeEl) timeEl.textContent = `${elapsed}ms`;
                    if (bodyEl) bodyEl.textContent = data;

                    // Wire up copy button for response
                    const copyBtn = document.getElementById(`copy-response-${endpointId}`);
                    if (copyBtn) {
    ...
    buildApiExplorerRequestConfig(endpoint, endpointId) {
        ...
        // Inputs are inside .parameter-inputs[data-endpoint-id] container
        const container = document.querySelector(`.parameter-inputs[data-endpoint-id="${endpointId}"]`);
        const paramInputs = container ? container.querySelectorAll('.param-input') : [];
        paramInputs.forEach(input => {
