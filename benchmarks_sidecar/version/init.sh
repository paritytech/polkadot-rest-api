...
# Sidecar Node/version endpoint benchmark
...
THREADS=$(jq -r ".benchmarks.version.scenarios[] | select(.name == \"$SCENARIO\") | .threads" "$CONFIG_FILE")
CONNECTIONS=$(jq -r ".benchmarks.version.scenarios[] | select(.name == \"$SCENARIO\") | .connections" "$CONFIG_FILE")
DURATION=$(jq -r ".benchmarks.version.scenarios[] | select(.name == \"$SCENARIO\") | .duration" "$CONFIG_FILE")
TIMEOUT=$(jq -r ".benchmarks.version.scenarios[] | select(.name == \"$SCENARIO\") | .timeout" "$CONFIG_FILE")
...
echo "Running Sidecar node/version endpoint benchmark: $SCENARIO"
...
wrk -d"$DURATION" -t"$THREADS" -c"$CONNECTIONS" --timeout "${TIMEOUT:-120s}" --latency \
    -s ./version.lua "${PROTOCOL}://$SERVER_HOST:$SERVER_PORT"
