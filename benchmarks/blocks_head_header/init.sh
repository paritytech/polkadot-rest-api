...
# Blocks head header endpoint benchmark
...
THREADS=$(jq -r ".benchmarks.blocks_head_header.custom_scenarios[]? | select(.name == \"$SCENARIO\") | .threads" "$CONFIG_FILE")
if [ -z "$THREADS" ] || [ "$THREADS" == "null" ]; then
    THREADS=$(jq -r ".standard_scenarios[] | select(.name == \"$SCENARIO\") | .threads" "$CONFIG_FILE")
fi

CONNECTIONS=$(jq -r ".benchmarks.blocks_head_header.custom_scenarios[]? | select(.name == \"$SCENARIO\") | .connections" "$CONFIG_FILE")
if [ -z "$CONNECTIONS" ] || [ "$CONNECTIONS" == "null" ]; then
    CONNECTIONS=$(jq -r ".standard_scenarios[] | select(.name == \"$SCENARIO\") | .connections" "$CONFIG_FILE")
fi

DURATION=$(jq -r ".benchmarks.blocks_head_header.custom_scenarios[]? | select(.name == \"$SCENARIO\") | .duration" "$CONFIG_FILE")
if [ -z "$DURATION" ] || [ "$DURATION" == "null" ]; then
    DURATION=$(jq -r ".standard_scenarios[] | select(.name == \"$SCENARIO\") | .duration" "$CONFIG_FILE")
fi

TIMEOUT=$(jq -r ".benchmarks.blocks_head_header.custom_scenarios[]? | select(.name == \"$SCENARIO\") | .timeout" "$CONFIG_FILE")
...
echo "Running blocks head header endpoint benchmark: $SCENARIO"
...
wrk -d"$DURATION" -t"$THREADS" -c"$CONNECTIONS" --timeout "${TIMEOUT:-120s}" --latency \
    -s ./blocks_head_header.lua "http://$SERVER_HOST:$SERVER_PORT"
