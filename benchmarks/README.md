...
## Benchmark Comparison Dashboard (Offline)

A Grafana dashboard for comparing `polkadot-rest-api` vs `substrate-api-sidecar` benchmark results side-by-side, using data from `bench_monitored.sh` runs.

### How it works

The `bench_monitored.sh` script produces JSON result files (throughput, latency, percentiles) and CSV resource files (RSS, CPU over time). A Python script loads these into a SQLite database, and Grafana reads the database to render the comparison dashboard.

```
bench_monitored.sh → JSON + CSV files
                          │
          load_results_to_sqlite.py
                          │
                          ▼
              grafana/data/benchmarks.db
                          │
                    docker compose up
                          │
                          ▼
...
Run `bench_monitored.sh` against both services, saving results to separate directories:

```bash
# Against polkadot-rest-api (e.g., port 8080)
./benchmarks/bench_monitored.sh --runs 10 8080 accounts_balance_info medium_load dedicated_server

# Against substrate-api-sidecar (e.g., port 8045)
./benchmarks/bench_monitored.sh --runs 10 8045 accounts_balance_info medium_load dedicated_server
```

Results are saved to `results/<benchmark>/` with JSON and CSV files per run.

### Step 2: Load data into SQLite

```bash
python3 benchmarks/grafana/load_results_to_sqlite.py \
  path/to/results-rest-api \
  path/to/results-sidecar
```

The script uses `Path(__file__).parent` to resolve the database path, so it can be run from any directory — the DB is always written to `benchmarks/grafana/data/benchmarks.db`.

Use `--db` to specify a different output path if needed:
...
**Note:** The script deletes the existing database and recreates it from scratch each time. This ensures clean data without duplicates.

### Step 3: Start Grafana

```bash
cd benchmarks/grafana
docker compose up -d
```

Open http://localhost:3000 (admin / admin). The "Benchmark Comparison: REST API vs Sidecar" dashboard loads automatically as the home dashboard.

Use the **Endpoint** and **Scenario** dropdowns at the top to select which benchmark to view.
...
The dashboard mirrors the layout of the HTML comparison reports generated by `polkadot-rest-checker`:

| Section | Panels |
|---------|--------|
| **Overview** | Median RPS, Median P99, Peak RSS (stat cards for each service) |
| **API Performance** | RPS per Run, Request Duration per Run, Max Request Duration per Run, Avg Latency per Run, Transfer Rate, Avg Response Size, Latency Percentiles, P99 per Run, Error Rate per Run, Summary Table |
| **Process Resources** | Memory (RSS) Over Time, CPU Over Time, Peak RSS per Run, Avg CPU per Run, Memory Growth Rate, Resource Summary |
| **Correlation** | Throughput vs Memory, Throughput vs CPU, Latency vs Memory, CPU vs Memory (per service) |
| **Per-Run Details** | Raw data table with all metrics per run |
